...
object ContentCommands {
  ...
  case class CreateContent(sessionId: UUID, content: Content, userId: UUID) extends ContentCommand

  case class DeleteContent(sessionId: UUID, id: UUID, userId: UUID) extends ContentCommand
}
